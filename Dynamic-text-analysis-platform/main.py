...
if __name__ == "__main__":
    ...
    # Semantic Topic Analysis (Knowledge-based)
    semantic_results = []
    for text in texts:
        scores = semantic_topic_scores(text)
        dominant_semantic_topic = max(scores, key=scores.get)
        semantic_results.append(dominant_semantic_topic)
        from collections import Counter
        semantic_counts = Counter(semantic_results)
    ...
    joblib.dump(final_lda_model, "models/lda_model.pkl")
    joblib.dump(vectorizer, "models/vectorizer.pkl")
    joblib.dump(topic_name_map, "models/topic_names.pkl")

    print("✅ Models saved for Web App")
    ...
    for doc_idx, topic_probs in enumerate(doc_topic_dist):
        ...
        semantic_topic = semantic_results[doc_idx]
        dominant_topics.append(dominant_topic_name)  
        print(f"  👉 LDA Topic: {dominant_topic_name}")
    ...
    # =======================
    # 📊 SEMANTIC TOPIC DISTRIBUTION GRAPH
    # =======================

    plt.figure(figsize=(10, 6))

    topics = list(semantic_counts.keys())
    counts = list(semantic_counts.values())

    plt.barh(topics, counts)
    plt.xlabel("Number of Documents")
    plt.ylabel("Semantic Topics")
    plt.title("Semantic Topic Distribution (Knowledge-Based)")
    plt.grid(axis="x")
    plt.tight_layout()
    plt.savefig("outputs/graphs/semantic_topic_distribution.png", dpi=300)
    plt.show()
    ...
    for i, text in enumerate(texts):
      ...
      #have to comment for pause distilbert
        ...
        dominant_topic = semantic_results[i]     # already computed earlier
        final_sentiment = vader_label          # choose VADER for insight
    ...
    plt.bar(labels, bert_values)
    plt.xlabel("Sentiment")
    plt.ylabel("Number of Documents")
    plt.title("Sentiment Distribution (DistilBERT)")
    plt.grid(axis="y")
    plt.tight_layout()
    plt.savefig("outputs/graphs/distilbert_sentiment_distribution.png", dpi=300)
    plt.show()
